...
uint8_t ps2_controller_read_data(void) {
    if (!ps2_controller_wait_read()) {
        kprintf("PS2 Controller: Read timeout!\n");
        return 0xFF;
    }
...
void ps2_controller_write_command(uint8_t cmd) {
    if (!ps2_controller_wait_write()) {
        kprintf("PS2 Controller: Write command timeout!\n");
        return;
    }
...
void ps2_controller_write_data(uint8_t data) {
    if (!ps2_controller_wait_write()) {
        kprintf("PS2 Controller: Write data timeout!\n");
        return;
    }
...
bool ps2_controller_flush_buffer(void) {
    ...
    while (inb(PS2_STATUS_PORT) & PS2_STATUS_OUTPUT_FULL) {
        ...
        count++;
        if (count > 32) {
            kprintf("PS2 Controller: Buffer flush overflow\n");
            return false;
        }
        io_wait();
    }
...
bool ps2_controller_set_config(uint8_t config) {
    ...
    if (ps2_controller_config != config) {
        kprintf("PS2 Controller: Config mismatch! Wrote 0x%02X, read 0x%02X\n", config, ps2_controller_config);
    }
    return true;
}
...
bool ps2_controller_init(void) {
    if (ps2_controller_initialized) {
        kprintf("PS2 Controller: Already initialized\n");
        return true;
    }
    
    kprintf("PS2 Controller: Initializing...\n");
    ...
    ps2_controller_config = ps2_controller_get_config();
    kprintf("PS2 Controller: Initial config: 0x%02X\n", ps2_controller_config);
    ...
    if (test_result != 0x55) {
        kprintf("PS2 Controller: Self-test failed (0x%02X)\n", test_result);
        return false;
    }
    ...
    if (test_result != 0x00) {
        kprintf("PS2 Controller: Port 1 test failed (0x%02X)\n", test_result);
        return false;
    }
    ...
    if (has_port2) {
        kprintf("PS2 Controller: Port 2 (mouse) detected\n");
        
        // Test port 2
        ps2_controller_write_command(PS2_CMD_TEST_PORT2);
        test_result = ps2_controller_read_data();
        if (test_result != 0x00) {
            kprintf("PS2 Controller: Port 2 test failed (0x%02X)\n", test_result);
            has_port2 = false;
        }
    } else {
        kprintf("PS2 Controller: No second port detected\n");
    }
    ...
    kprintf("PS2 Controller: Initialization complete (config: 0x%02X)\n", ps2_controller_config);
